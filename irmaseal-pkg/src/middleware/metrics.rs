...
mod tests {
    ...
    use irmaseal_core::{Policy, Attribute};
    use irmaseal_core::api::{Parameters, KeyResponse};
    ...
    async fn test_get_metrics() {
        ...
        // First request
        let header = (header_name.clone(), HeaderValue::from_static("Outlook,1234.5678.90,pg4ol,0.0.1"));
        let req = test::TestRequest::get().uri("/v2/parameters").insert_header(header).to_request();
        let kr: Parameters<CGWKV> = test::call_and_read_body_json(&app, req).await;
        assert_eq!(&kr.public_key.0, &pk);
        
        // Second request
        let header = (header_name.clone(), HeaderValue::from_static("Thunderbird,1234.5678.90,pg4tb,0.0.2"));
        let req = test::TestRequest::get().uri("/v2/parameters").insert_header(header).to_request();
        let kr: Parameters<CGWKV> = test::call_and_read_body_json(&app, req).await;
        assert_eq!(&kr.public_key.0, &pk);
        ...
        // Fourth request
        let header = (header_name, HeaderValue::from_static("Outlook,1234.5678.90,pg4ol,0.0.1"));
