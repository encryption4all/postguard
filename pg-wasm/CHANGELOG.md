...
The section below is the record from when `pg-wasm` still had a tag series of
its own, ending at `pg-wasm-v0.5.5`. It is real published history and is kept
deliberately: "no per-version entries" above is a rule about versions from
`0.5.6` onward, not a licence to delete the one entry that predates it.
