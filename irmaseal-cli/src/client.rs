...
impl<'a> Client<'a> {
    ...
    pub async fn request_start(&self, kr: &KeyRequest) -> Result<irma::SessionData, ClientError> {
        self.client
            .post(self.create_url("v2/irma/request/start"))
            .json(kr)
            .send()
            .await?
            .error_for_status()?
    ...
    pub async fn request_jwt(&self, token: &irma::SessionToken) -> Result<String, ClientError> {
        self.client
            .get(self.create_url(&format!("v2/irma/request/jwt/{}", token.0)))
            .send()
            .await?
            .error_for_status()?
            .text()
    ...
    {
        self.client
            .get(self.create_url(&format!("v2/irma/request/key/{timestamp}")))
            .bearer_auth(auth)
            .send()
            .await?
            .error_for_status()?
